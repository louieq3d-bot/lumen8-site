...
(function () {
  ...
  W.funnel = (el, F) => {
    ...
    draw('global'); tabs(el, 'funnel', draw);
  };
  /* reach: four tiers from the entry market to the global platform, widths on a log scale of the preparation
     spend each can address; click a tier for its figures */
  W.ladder = (el, L) => {
    el.innerHTML = ''; el.classList.add('reach'); const list = document.createElement('div'); list.className = 'reach-list'; const info = document.createElement('div'); info.className = 'ladder-info'; el.append(list, info);
    const hd = document.createElement('div'); hd.className = 'reach-h'; hd.innerHTML = '<b>A global platform, entered through one market</b><span>Preparation spend the engine can address, USD bn a year · widths on a log scale · the tier we are in now is lit</span>'; list.appendChild(hd);
    const cs = [COL.cyan, COL.blue2, COL.green, COL.silver]; const lo = Math.log10(Math.min(...L.map((r) => r.tam))), hi = Math.log10(Math.max(...L.map((r) => r.tam)));
    const rows = L.map((r, i) => { const w = 18 + 82 * (Math.log10(r.tam) - lo) / Math.max(1e-6, hi - lo); const d = document.createElement('button'); d.type = 'button'; d.className = 'reach-row'; d.style.setProperty('--c', cs[i]); d.innerHTML = '<i>0' + (i + 1) + '</i><div class="rt"><b>' + esc(r.title) + '</b><span>' + esc(r.sub) + '</span></div><div class="rb"><em style="width:' + w.toFixed(1) + '%"></em></div><div class="rv"><b>' + esc(r.tamLabel) + '</b><span>' + esc(r.status) + '</span></div>'; d.addEventListener('click', () => sel(i)); tipOn(d, '<b>' + esc(r.title) + ' · ' + esc(r.tamLabel) + '</b>' + esc(r.sub)); list.appendChild(d); return d; });
    const sel = (i) => { rows.forEach((d, k) => d.classList.toggle('on', k === i)); const r = L[i]; info.style.setProperty('--c', cs[i]); info.innerHTML = '<span class="sub">' + esc(r.status) + '</span><h3>' + esc(r.title) + '</h3><p>' + esc(r.text) + '</p><div class="figs">' + r.figures.map((f) => '<div><b>' + esc(f[0]) + '</b><span>' + esc(f[1]) + '</span></div>').join('') + '</div>'; };
    sel(0);
  };
  ...
  W.sim = (el, S0) => {
    ...
    const ctl = [['sitesY2', 'Sites prepared in year 2', 10, 200, 5, (v) => v + ' sites'], ['fee', 'Average fee per feasibility study', 10, 80, 5, (v) => 'USD ' + v + 'k'], ['team', 'Team licences', 0, 30, 1, (v) => v], ['teamFee', 'Team licence', 12, 30, 1, (v) => 'USD ' + v + 'k / yr'], ['ent', 'Enterprise licences', 0, 12, 1, (v) => v], ['entFee', 'Enterprise licence', 150, 500, 10, (v) => 'USD ' + v + 'k / yr'], ['inst', 'Institutional licences', 0, 3, 1, (v) => v], ['instFee', 'Institutional licence', 500, 1500, 50, (v) => 'USD ' + v + 'k / yr'], ['assets', 'Assets under monitoring', 0, 200, 5, (v) => v + ' assets'], ['monFee', 'Monitoring fee per asset', 2, 10, 1, (v) => 'USD ' + v + 'k / yr']];
    el.innerHTML = '<div class="ctl">' + ctl.map((c) => '<label><span>' + esc(c[1]) + '<b data-v="' + c[0] + '"></b></span><input type="range" data-k="' + c[0] + '" min="' + c[2] + '" max="' + c[3] + '" step="' + c[4] + '" value="' + S[c[0]] + '"></label>').join('') + '<div class="simbtns"><button class="btn sm ghost reset"><span>Reset to the plan</span></button><button class="btn sm ghost pin"><span>Pin as scenario A</span></button></div></div><div class="out"><div class="outk"><div><b data-o="arr"></b><span>annual recurring revenue, month-24 run-rate</span></div><div><b data-o="prep"></b><span>one-off preparation revenue in year 2</span></div><div><b data-o="rec"></b><span>recurring share of year-2 revenue</span></div><div><b data-o="cmr"></b><span>capex prepared to one standard in year 2, at USD 1.7 m a site</span></div></div><div class="stack"></div><div class="pinned" hidden></div><p class="srcline">Plan landing at month 24: USD 1.0 m ARR, 8 customers including the first enterprise licences, 52 sites cumulative at USD 25–40k a study, 30 assets under monitoring. Every input is a slider so the landing can be argued with. This is a model, not a forecast.</p></div>';
  ...
  /* the revenue plan: stacked years, ARR line, customers and sites, enterprise value at the reader's multiple */
  W.plan = (el, PL) => {
    el.innerHTML = '<div class="plan-ctl"><label><span>Offtake against the plan<b data-v="g"></b></span><input type="range" data-k="g" min="50" max="160" step="5" value="100"></label><label><span>Multiple on exit-year revenue<b data-v="m"></b></span><input type="range" data-k="m" min="6" max="30" step="1" value="18"></label></div><div class="plan-chart chart"></div><div class="plan-years"></div><div class="plan-val"><div class="pv-h"><i>Enterprise value at the reader\'s multiple</i><span>exit-year revenue × multiple · USD m</span></div><div class="pv-grid"></div><div class="pv-scen"></div></div><p class="srcline"></p>';
    $('.srcline', el).textContent = PL.caveat;
    const S = { g: 1, m: 18 }; const yrs = PL.years; const rv = (a) => a.prep + a.lic + a.mon;
    ...
    const draw = () => {
      ...
      const lg = svgEl('text', { x: L, y: H - 6, class: 'sm' }, svg); lg.textContent = 'COLUMNS: REVENUE RECOGNISED IN THE YEAR, USD m (LEFT) · LINE: RECURRING REVENUE AT YEAR END (RIGHT)';
      $('.plan-years', el).innerHTML = yrs.map((y, i) => { const a = A[i]; return '<div class="py"><div class="py-h"><b>' + esc(y.y) + '</b><span>' + esc(y.label) + '</span></div><div class="py-k"><div><b>USD ' + (a.prep + a.lic + a.mon).toFixed(1) + ' m</b><span>revenue</span></div><div><b>USD ' + a.arr.toFixed(1) + ' m</b><span>ARR exit</span></div><div><b>' + a.customers + '</b><span>customers</span></div><div><b>' + a.sites + '</b><span>sites in year</span></div><div><b>' + a.assets + '</b><span>assets monitored</span></div><div><b>' + y.markets + '</b><span>markets</span></div></div><p>' + esc(y.note) + '</p></div>'; }).join('');
      $('.pv-grid', el).innerHTML = A.map((a, i) => '<div><i>' + esc(yrs[i].y) + '</i><b>USD ' + (rv(a) * S.m).toFixed(0) + ' m</b><span>' + rv(a).toFixed(1) + ' × ' + S.m + '</span></div>').join('');
      const a3 = A[2], a4 = A[3]; $('.pv-scen', el).innerHTML = '<div class="pv-sh">Where the company could stand after three and four years, at three multiples</div>' + PL.multiples.map(([nm, m]) => '<div class="pv-row"><span>' + esc(nm) + ' · ' + m + '× revenue</span><div><i style="width:' + (100 * m / 30).toFixed(1) + '%"></i></div><b>USD ' + (rv(a3) * m).toFixed(0) + ' m <small>year 3</small></b><b>USD ' + (rv(a4) * m).toFixed(0) + ' m <small>year 4</small></b></div>').join('') + '<div class="pv-note">Against the USD 5 m of platform equity in this round. The three scenarios sit where AI-native software with proprietary data is priced, 12–25× revenue; the public SaaS bands on the next page are shown beside them so the premium is argued, not assumed.</div>';
      $('[data-v=g]', el).textContent = Math.round(S.g * 100) + '% of plan'; $('[data-v=m]', el).textContent = S.m + '× revenue';
    };
  ...
  W.entry = (el, PL) => {
    el.innerHTML = '<div class="entry-ctl"><label><span>Post-money at entry<b data-v></b></span><input type="range" min="10" max="60" step="1" value="21"></label><div class="entry-note">Carta\'s Q3 2025 median seed pre-money was USD 16 m; USD 5 m on top of that is the default. The plan\'s enterprise value is exit-year revenue times the multiple, at the three multiples where AI-native software is priced.</div></div><div class="entry-grid"></div>';
    const rng = $('input', el); const rv = (a) => a.prep + a.lic + a.mon; const a3 = rv(PL.years[2]), a4 = rv(PL.years[3]);
    const draw = () => { const pm = +rng.value; $('[data-v]', el).textContent = 'USD ' + pm + ' m · ' + (100 * 5 / pm).toFixed(1) + '% for USD 5 m'; $('.entry-grid', el).innerHTML = PL.multiples.map(([nm, m]) => { const e3 = a3 * m, e4 = a4 * m; return '<div class="ecell"><i>' + esc(nm) + ' · ' + m + '× revenue</i><div class="erow"><span>Year 3</span><b>USD ' + e3.toFixed(0) + ' m</b><em>' + (e3 / pm).toFixed(1) + '× entry</em></div><div class="erow"><span>Year 4</span><b>USD ' + e4.toFixed(0) + ' m</b><em>' + (e4 / pm).toFixed(1) + '× entry</em></div><div class="ebar"><i style="width:' + Math.min(100, 100 * (e4 / pm) / 60).toFixed(1) + '%"></i></div></div>'; }).join(''); };
  ...
  W.docs = (el) => {
    ...
    el.innerHTML = docs.length ? docs.map((d) => d.soon ? '<div class="doc soon" aria-disabled="true"><i>PDF</i><div><b>' + esc(d.title) + '</b><span>' + esc(d.sub) + '</span></div><div class="acts"><span class="soonchip">Coming soon</span></div></div>' : '<div class="doc"><i>PDF</i><div><b>' + esc(d.title) + '</b><span>' + esc(d.sub) + ' · ' + (d.size / 1048576).toFixed(1) + ' MB</span></div><div class="acts"><button data-open="' + esc(d.slug) + '">Open</button><button data-save="' + esc(d.slug) + '">Save</button></div></div>').join('') : '<p class="srcline">No documents in this build.</p>';
