...
(function () {
  ...
  W.plan = (el, PL) => {
    el.innerHTML = '<div class="plan-ctl"><label><span>Offtake against the plan<b data-v="g"></b></span><input type="range" data-k="g" min="50" max="160" step="5" value="100"></label><label><span>Multiple on exit-year recurring revenue<b data-v="m"></b></span><input type="range" data-k="m" min="4" max="25" step="1" value="12"></label></div><div class="plan-chart chart"></div><div class="plan-years"></div><div class="plan-val"><div class="pv-h"><i>Enterprise value at the reader\'s multiple</i><span>exit-year ARR × multiple · USD m</span></div><div class="pv-grid"></div><div class="pv-scen"></div></div><p class="srcline"></p>';
    $('.srcline', el).textContent = PL.caveat;
    const S = { g: 1, m: 12 }; const yrs = PL.years;
    ...
    const draw = () => {
      ...
      $('.pv-grid', el).innerHTML = A.map((a, i) => '<div><i>' + esc(yrs[i].y) + '</i><b>USD ' + (a.arr * S.m).toFixed(0) + ' m</b><span>' + a.arr.toFixed(1) + ' × ' + S.m + '</span></div>').join('');
      const a3 = A[2], a4 = A[3]; $('.pv-scen', el).innerHTML = '<div class="pv-sh">Where the company could stand after three and four years, at three multiples</div>' + PL.multiples.map(([nm, m]) => '<div class="pv-row"><span>' + esc(nm) + ' · ' + m + '× ARR</span><div><i style="width:' + (100 * a4.arr * m / (a4.arr * 25)).toFixed(1) + '%"></i></div><b>USD ' + (a3.arr * m).toFixed(0) + ' m <small>year 3</small></b><b>USD ' + (a4.arr * m).toFixed(0) + ' m <small>year 4</small></b></div>').join('') + '<div class="pv-note">Against the USD 5 m of platform equity in this round. Multiples are the reader\'s; the plan\'s recurring revenue is the model\'s.</div>';
      $('[data-v=g]', el).textContent = Math.round(S.g * 100) + '% of plan'; $('[data-v=m]', el).textContent = S.m + '× ARR';
    };
